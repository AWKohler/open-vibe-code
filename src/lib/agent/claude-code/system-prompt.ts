...
export function buildClaudeCodeAppendPrompt(input: BuildAppendPromptInput): string {
  ...
  if (hasBackend) {
    ...
    sections.push(
      ...
      "   IMPORTANT for subscriptions/paywalls/tiers: also pass `metadata: { appUserId: <the signed-in user's _id> }` to `createCheckoutSession`. That is the ONLY reliable way the webhook maps the subscription back to the user — it returns as `event.data.metadata.appUserId` in `billing.ts`. Stripe subscription events carry NO email, so never match on `customerEmail`.",
